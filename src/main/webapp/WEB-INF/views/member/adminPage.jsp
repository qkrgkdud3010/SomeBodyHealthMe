...
<div class="recent-diet-requests">
    <div class="section-header">
        <h3>식단 등록 요청 내역</h3>
        <button class="more-btn" onclick="location.href='${pageContext.request.contextPath}/mydiet/adminDietPlan.do'">더보기</button>
    </div>
    <table>
        <thead>
            <tr>
        ...
        <tbody>
            <c:choose>
                <c:when test="${not empty recentDietPlans}">
                    <c:forEach var="diet" items="${recentDietPlans}">
                        <tr>
                            <td>${diet.dietId}</td>
                            <td>${diet.foodName}</td>
                            <td>${diet.userNum}</td>
                            <td>
                                <c:choose>
                                    <c:when test="${diet.dietComment == 1}">요청 중</c:when>
                                    <c:when test="${diet.dietComment == 2}">반려됨</c:when>
